...
class _AdminPageState extends State<AdminPage> {
  ...
  Future<void> _setCarStatus(String id, String status) async {
    ...
    try { await _db.collection('cars').doc(id).update({'status': status, 'reviewedAt': FieldValue.serverTimestamp()}); _toast(status == 'approved' ? 'تم قبول الإعلان ✓' : 'تم رفض الإعلان'); }
  ...
  Future<void> _setRequestStatus(String id, String status) async {
    setState(() => _loading = true);
    try { await _db.collection('purchaseRequests').doc(id).update({'status': status, 'reviewedAt': FieldValue.serverTimestamp()}); _toast(status == 'approved' ? 'تمت الموافقة على الطلب ✓' : 'تم رفض الطلب'); }
  ...
  Widget _cars() => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
    ...
    builder: (context, snap) {
      ...
      return ListView.builder(padding: const EdgeInsets.fromLTRB(16, 8, 16, 28), itemCount: docs.length, itemBuilder: (_, i) {
        ...
        return _GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          if (images.isNotEmpty) ClipRRect(borderRadius: BorderRadius.circular(16), child: Image.network(images.first, height: 185, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const SizedBox(height: 185, child: Icon(Icons.directions_car, size: 60, color: _gold)))) else const SizedBox(height: 120, child: Center(child: Icon(Icons.directions_car, size: 60, color: _gold))),
          const SizedBox(height: 12), Text('${d['name'] ?? 'سيارة'} • ${d['year'] ?? ''}', style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900)), const SizedBox(height: 7),
          Text('السعر: ${d['price'] ?? '-'}\nالمدينة: ${d['city'] ?? '-'}\nالنوع: ${d['type'] ?? '-'}\nالبائع: ${d['sellerId'] ?? '-'}', style: const TextStyle(color: _muted, height: 1.5)), const SizedBox(height: 14),
          Row(children: [Expanded(child: FilledButton.icon(onPressed: _loading ? null : () => _setCarStatus(doc.id, 'approved'), icon: const Icon(Icons.check), label: const Text('قبول'))), const SizedBox(width: 10), Expanded(child: OutlinedButton.icon(onPressed: _loading ? null : () => _setCarStatus(doc.id, 'rejected'), icon: const Icon(Icons.close), label: const Text('رفض')))]),
        ]));
      });
    },
  );
  ...
  Widget _requests() => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
    ...
    builder: (context, snap) {
      ...
      return ListView.separated(padding: const EdgeInsets.fromLTRB(16, 8, 16, 28), itemCount: docs.length, separatorBuilder: (_, __) => const SizedBox(height: 10), itemBuilder: (_, i) {
        final doc = docs[i]; final d = doc.data(); final status = '${d['status'] ?? 'pending'}'; final short = doc.id.substring(0, doc.id.length > 7 ? 7 : doc.id.length);
        return _GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [Row(children: [const CircleAvatar(backgroundColor: Color(0x1FFFB51B), child: Icon(Icons.shopping_cart_outlined, color: _gold)), const SizedBox(width: 10), Expanded(child: Text('طلب شراء #$short', style: const TextStyle(fontWeight: FontWeight.w900))), _StatusBadge(status: status)]), const SizedBox(height: 12), Text('المشتري: ${d['buyerId'] ?? '-'}\nالبائع: ${d['sellerId'] ?? '-'}\nالسيارة: ${d['carId'] ?? '-'}', style: const TextStyle(color: _muted, height: 1.5, fontSize: 12)), if (status == 'pending') ...[const SizedBox(height: 12), Row(children: [Expanded(child: FilledButton.icon(onPressed: _loading ? null : () => _setRequestStatus(doc.id, 'approved'), icon: const Icon(Icons.check), label: const Text('موافقة'))), const SizedBox(width: 10), Expanded(child: OutlinedButton.icon(onPressed: _loading ? null : () => _setRequestStatus(doc.id, 'rejected'), icon: const Icon(Icons.close), label: const Text('رفض')))])]]));
      });
    },
  );
}
...
class _DashboardSummary extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => SizedBox(height: 92, child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(stream: db.collection('users').snapshots(), builder: (_, users) => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(stream: db.collection('purchaseRequests').where('status', isEqualTo: 'pending').snapshots(), builder: (_, requests) => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(stream: db.collection('cars').where('status', isEqualTo: 'pending').snapshots(), builder: (_, cars) { final stats = [('المستخدمون', users.data?.size ?? 0, Icons.people_outline), ('طلبات جديدة', requests.data?.size ?? 0, Icons.shopping_bag_outlined), ('إعلانات معلقة', cars.data?.size ?? 0, Icons.directions_car_outlined)]; return ListView.separated(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), itemCount: stats.length, separatorBuilder: (_, __) => const SizedBox(width: 10), itemBuilder: (_, i) => Container(width: 145, padding: const EdgeInsets.all(13), decoration: BoxDecoration(color: _panel.withOpacity(.94), borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.white10)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(stats[i].$3, color: _gold, size: 20), const Spacer(), Text('${stats[i].$2}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)), Text(stats[i].$1, style: const TextStyle(color: _muted, fontSize: 11))])); })); })));
}
class _GlassCard extends StatelessWidget { final Widget child; const _GlassCard({required this.child}); @override Widget build(BuildContext context) => Card(color: _panel.withOpacity(.96), margin: const EdgeInsets.only(bottom: 4), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(22), side: const BorderSide(color: Colors.white10)), child: Padding(padding: const EdgeInsets.all(14), child: child)); }
