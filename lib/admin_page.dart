...
class _AdminPageState extends State<AdminPage> {
  ...
  Future<void> _setRequestStatus(String id, String status) async {
    setState(() => _loading = true);
    try {
      await _db.collection('purchaseRequests').doc(id).update({
        'status': status,
        'reviewedAt': FieldValue.serverTimestamp(),
      });
      _toast(status == 'approved' ? 'تمت الموافقة على الطلب ✓' : 'تم رفض الطلب');
    } catch (e) {
      _toast('تعذر تحديث الطلب');
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }
            ...
            return _GlassCard(
              child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                if (images.isNotEmpty)
                  ClipRRect(borderRadius: BorderRadius.circular(16), child: Image.network(images.first, height: 185, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const SizedBox(height: 185, child: Icon(Icons.directions_car, size: 60, color: _gold))))
                else
                  const SizedBox(height: 120, child: Center(child: Icon(Icons.directions_car, size: 60, color: _gold))),
                const SizedBox(height: 12),
                Text('${data['name'] ?? 'سيارة'} • ${data['year'] ?? ''}', style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900)),
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snap) {
        ...
        docs.sort((a, b) {
          ...
          if (aa is Timestamp && bb is Timestamp) return bb.compareTo(aa);
          return 0;
        });
          ...
          itemBuilder: (_, index) {
            ...
            return _GlassCard(
              child: ListTile(
                ...
                subtitle: Text('${data['phone'] ?? '-'}\nUID: ${docs[index].id}', style: const TextStyle(color: _muted, fontSize: 11)),
                trailing: _RoleBadge(role: role.toString()),
              ),
            );
          },
        );
            ...
            return _GlassCard(
              child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                Row(children: [
                  ...
                  const SizedBox(width: 10),
                  Expanded(child: Text('طلب شراء #${doc.id.substring(0, doc.id.length > 7 ? 7 : doc.id.length)}', style: const TextStyle(fontWeight: FontWeight.w900))),
                ...
                if (status == 'pending') ...[
                  ...
                  Row(children: [
                    ...
                    const SizedBox(width: 10),
                    Expanded(child: OutlinedButton.icon(onPressed: _loading ? null : () => _setRequestStatus(doc.id, 'rejected'), icon: const Icon(Icons.close), label: const Text('رفض'))),
                  ]),
                ],
              ]),
            );
      ...
      child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (_, users) => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (_, requests) => StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (_, cars) {
              final stats = [
                ('المستخدمون', users.data?.size ?? 0, Icons.people_outline),
                ('طلبات جديدة', requests.data?.size ?? 0, Icons.shopping_bag_outlined),
                ('إعلانات معلقة', cars.data?.size ?? 0, Icons.directions_car_outlined),
              ];
              return ListView.separated(
                ...
                itemBuilder: (_, i) => Container(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Icon(stats[i].$3, color: _gold, size: 20),
                    const Spacer(),
                    Text('${stats[i].$2}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
                    Text(stats[i].$1, style: const TextStyle(color: _muted, fontSize: 11)),
                  ]),
                ),
              );
            },
...
class _GlassCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Card(color: _panel, margin: const EdgeInsets.only(bottom: 4), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(22), side: const BorderSide(color: Colors.white10)), child: Padding(padding: const EdgeInsets.all(14), child: child));
}
...
class _RoleBadge extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 6), decoration: BoxDecoration(color: _gold.withOpacity(.13), borderRadius: BorderRadius.circular(20)), child: Text(role == 'admin' ? 'أدمن' : 'مستخدم', style: const TextStyle(color: _gold, fontSize: 10, fontWeight: FontWeight.w800)));
}
...
class _EmptyState extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, color: _gold, size: 52), const SizedBox(height: 12), Text(text, style: const TextStyle(color: _muted))]));
}

class _ErrorState extends StatelessWidget {
  final String text;
  const _ErrorState(this.text);
  @override
  Widget build(BuildContext context) => Center(child: Padding(padding: const EdgeInsets.all(22), child: Text(text, textAlign: TextAlign.center, style: const TextStyle(color: Colors.redAccent))));
}
