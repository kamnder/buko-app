                  ...
                  child: ConstrainedBox(
                    ...
                    child: Column(
                      children: [
                        ...
                        const Text(
                          'BUKO',
                          style: TextStyle(fontSize: 42, fontWeight: FontWeight.w900, letterSpacing: 2),
                        ),
                        const Text(
                            ...
                            child: Column(
                              children: [
                                ...
                                const Align(
                                  ...
                                  child: Text(
                                    'دخول آمن بدون SMS أو بريد إلكتروني',
                                ...
                                TextButton(
                                  ...
                                  child: Text(
                                    register ? 'لدي حساب بالفعل — تسجيل الدخول' : 'ليس لدي حساب — إنشاء حساب',
                                  ),
                                ),
                                if (error.isNotEmpty)
                                  Padding(
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (_, __) => CustomPaint(
        painter: _AuthScenePainter(controller.value),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final sky = Paint()
      ..shader = const LinearGradient(
        ...
        colors: [Color(0xFF101A29), Color(0xFF8A5034), Color(0xFF0A0E13)],
    ...
    canvas.drawCircle(
      Offset(w * .78, h * .22),
      math.min(w, h) * .055,
      Paint()..color = const Color(0xFFFFB51B).withOpacity(.45),
    );

    final city = Paint()..color = const Color(0xFF080D13).withOpacity(.78);
    for (var i = 0; i < 13; i++) {
      final bh = 42.0 + (i % 5) * 17.0;
      final bw = 22.0 + (i % 3) * 12.0;
      canvas.drawRect(Rect.fromLTWH(i * w / 12 - 8, h * .57 - bh, bw, bh), city);
    }
    ...
    final lane = Paint()
      ...
      ..strokeWidth = math.max(2, w * .006);
    for (var i = -2; i < 10; i++) {
      final x = (i * w * .19 + t * w * .65) % (w + 120) - 60;
      canvas.drawLine(Offset(x, h * .91), Offset(x + w * .08, h * .76), lane);
    }
  ...
  void _drawCar(Canvas canvas, Offset p, double z, bool hero) {
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(p.dx - 64 * z, p.dy - 22 * z, 128 * z, 39 * z),
        Radius.circular(13 * z),
      ),
      body,
    );
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(p.dx - 36 * z, p.dy - 39 * z, 67 * z, 29 * z),
    ...
    canvas.drawCircle(Offset(p.dx + 42 * z, p.dy + 17 * z), 12 * z, wheel);
    canvas.drawLine(
      Offset(p.dx - 50 * z, p.dy - 2 * z),
      Offset(p.dx + 50 * z, p.dy - 2 * z),
      trim..strokeWidth = 3 * z,
    );
    ...
    if (hero) {
      ...
      canvas.drawOval(
        Rect.fromCenter(center: Offset(p.dx + 2 * z, p.dy - 59 * z), width: 35 * z, height: 19 * z),
        cloth,
      );
      canvas.drawOval(
        Rect.fromCenter(center: Offset(p.dx - 5 * z, p.dy - 64 * z), width: 25 * z, height: 10 * z),
        cloth,
      );
      canvas.drawLine(
        Offset(p.dx - 7 * z, p.dy - 41 * z),
        Offset(p.dx - 20 * z, p.dy - 25 * z),
        dark..strokeWidth = 6 * z,
      );
    }
  }
